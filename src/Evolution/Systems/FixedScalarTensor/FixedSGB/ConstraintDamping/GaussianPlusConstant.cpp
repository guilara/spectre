...
#include "Evolution/Systems/FixedScalarTensor/FixedSGB/ConstraintDamping/GaussianPlusConstant.hpp"
...
#define INSTANTIATE(_, data)                                                 \
  template fe::sgb::ConstraintDamping::GaussianPlusConstant<                 \
      DIM(data), FRAME(data)>::GaussianPlusConstant(CkMigrateMessage* msg);  \
  template fe::sgb::ConstraintDamping::                                      \
      GaussianPlusConstant<DIM(data), FRAME(data)>::GaussianPlusConstant(    \
          const double constant, const double amplitude, const double width, \
          const std::array<double, DIM(data)>& center);                      \
  template void fe::sgb::ConstraintDamping::GaussianPlusConstant<            \
      DIM(data), FRAME(data)>::pup(PUP::er& p);                              \
  template auto fe::sgb::ConstraintDamping::GaussianPlusConstant<            \
...
#define INSTANTIATE(_, data)                                               \
  template void fe::sgb::ConstraintDamping::                               \
