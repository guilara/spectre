...
#include "Evolution/Systems/FixedScalarTensor/FixedDecoupledScalar/ConstraintDamping/GaussianPlusConstant.hpp"
...
#define INSTANTIATE(_, data)                                                  \
  template fe::DecoupledScalar::ConstraintDamping::GaussianPlusConstant<      \
      DIM(data), FRAME(data)>::GaussianPlusConstant(CkMigrateMessage* msg);   \
  template fe::DecoupledScalar::ConstraintDamping::                           \
      GaussianPlusConstant<DIM(data), FRAME(data)>::GaussianPlusConstant(     \
          const double constant, const double amplitude, const double width,  \
          const std::array<double, DIM(data)>& center);                       \
  template void fe::DecoupledScalar::ConstraintDamping::GaussianPlusConstant< \
      DIM(data), FRAME(data)>::pup(PUP::er& p);                               \
  template auto fe::DecoupledScalar::ConstraintDamping::GaussianPlusConstant< \
...
#define INSTANTIATE(_, data)                                               \
  template void fe::DecoupledScalar::ConstraintDamping::                   \
